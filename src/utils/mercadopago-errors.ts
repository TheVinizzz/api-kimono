 ...
 */
export function processMercadoPagoError(error: any): {
  message: string;
  code?: string;
  statusCode: number;
  isRetryable: boolean;
  ...
  if (status === 400 && data?.cause?.length > 0) {
    const cause = data.cause[0];
    const errorCode = cause.code?.toString();
    
    // Buscar mensagem específica
    const specificMessage = MERCADOPAGO_ERROR_CODES[errorCode as keyof typeof MERCADOPAGO_ERROR_CODES];
  ...
  // Erro genérico
  return {
    message: data?.message || 'Erro no processamento do pagamento',
 ...
 */
export function validateCPFForMercadoPago(cpf: string): boolean {
 ...
 */
export function formatPhoneForMercadoPago(phone: string): { area_code: string; number: string } {
 ...
 */
export function generateExternalReference(orderId: number, type: 'guest' | 'user' = 'user'): string {
